    ...
    class Handler_autoPipe : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication app)
        {
            try
            {
                ...
                foreach (List<string> rows in mhx.MHdata)
                {
                    UIDocument edit_uidoc = app.OpenAndActivateDocument(@"D:\SinoPipe-RFA\edit.rfa");
                    ...
                    using (Transaction t = new Transaction(edit_doc, "Create sphere direct shape"))
                    {
                            ...
                            else if (rows[4] == "方形")
                            {
                                ...
                                else
                                {
                                    try
                                    {
                                        ...
                                        double ccd; // 中心距
                                        if (radius * 2 <= 50) ccd = 100;
                                        else if (radius * 2 <= 100) ccd = 180;
                                        else if (radius * 2 <= 150) ccd = 210;
                                        else ccd = 310;
                                       
                                        double PB1 = (B - ccd * (Xn - 1)) / 2;
                                        double PB2 = (H - ccd * (Yn - 1)) / 2;
                        ...
                        edit_doc.SaveAs(@"D:\SinoPipe-RFA\realcase\edit" + rows[1] + ".rfa", saveAsOptions);
                    ...
                    using (Transaction t = new Transaction(doc, "load family"))
                    {
                        ...
                        try
                        {
                            doc.LoadFamily(@"D:\SinoPipe-RFA\realcase\edit" + rows[1] + ".rfa");
                        }
                        catch
                        {
        ...
        { 
            using (Transaction t = new Transaction(doc, "make profile xn * yn"))
            {
                ...
                else if (xy.Count == 9) //方形
                {
                    ...
                    center_dist1 = (double.Parse(xy[2]) - 2 * double.Parse(xy[4])) / (int.Parse(xy[0]) - 1);
                    center_dist2 = (double.Parse(xy[3]) - 2 * double.Parse(xy[5])) / (int.Parse(xy[1]) - 1);
                    /*
                    double PB1 = (double.Parse(xy[2]) - 210 * (int.Parse(xy[0]) - 1)) / 2;
                    double PB2 = (double.Parse(xy[3]) - 210 * (int.Parse(xy[1]) - 1)) / 2;
